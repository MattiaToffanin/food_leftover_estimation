...
std::vector<cv::Rect> getFoodRect(const std::vector<cv::Vec3f> dishes_set, const cv::Mat &img);							//get Rectangle containing the dishes
std::vector<cv::Mat> getRegionMask(const std::vector<cv::Vec3f> dishes_set, const cv::Size size);		//get the mask of the dishes
